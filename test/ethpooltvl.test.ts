...
const standardUri = "https://test.com/{id}.png";
...
contract("Deploy Eth Pool Faucet", (accounts) => {
  beforeEach(async () => {
    ...
    await faucet.initialize(accounts[0], standardUri, adapter_address);
    await ethPoolAdapter.addApprovedAdmin(faucet.address, {from: accounts[0]});
  });
  ...
  it("should be able to get the correct adapter pool address", async () => {
    let fetchedPoolAddress = await faucet.getPoolAddress.call({from: accounts[0]});
    expect(fetchedPoolAddress).to.equal(rariPoolController);
  });

  it("should be able to set the correct pool address from the owner", async () => {
    let fetchedPoolAddress = await faucet.setPoolAddress.call(updatedPoolAddress, {from: accounts[0]});
    expect(fetchedPoolAddress).to.equal(updatedPoolAddress);
  });
  ...
  it("creates tranche from contract owner level 1", async function () {
    ...
    let res = await faucet.createTranche.call(1, [1, 2, 3], standardUri, true, {from: accounts[0]});
    expect(res.toString()).to.equal('1');
  });

  it("should not be able to create tranche if not the owner", async () => {
    await ShouldThrow(faucet.createTranche.call(1, [1, 2, 3], standardUri, true, {from: accounts[1]}));
  });
  ...
  it("should be able to get if tranche exists", async () => {
    ...
    await faucet.createTranche(1, [1], standardUri, true, {from: accounts[0]});

    // * Fetch if tranche exists from owner
    let trancheExists = await faucet.getTrancheExists(1, {from: accounts[0]});
    expect(trancheExists.toString()).to.equal('true');

    // * Fetch if tranche exists from user
    trancheExists = await faucet.getTrancheExists(1, {from: accounts[1]});
    expect(trancheExists.toString()).to.equal('true');
  });
  ...
  it("should be able to get the correct tranche uri", async () => {
    ...
    let fetchedUri = await faucet.getTrancheUri(1, {from: accounts[0]});
    expect(fetchedUri).to.equal(standardUri);
  });

  it("owner should be able to set tranche uri", async () => {
    // * Expect uninitialized tranche get enabled to fail
    await ShouldThrow(faucet.setTrancheUri.call(1, standardUri, {from: accounts[0]}));
    await ShouldThrow(faucet.setTrancheUri.call(1, standardUri, {from: accounts[1]}));
    ...
    let tranche_level = await faucet.setTrancheUri.call(1, standardUri, {from: accounts[0]});
  ...
  it("should be able to get and set user tranche level", async () => {
    ...
    let userTrancheLevel = await faucet.getUserTrancheLevel.call(accounts[1], { from: accounts[0] });
    expect(userTrancheLevel.toString()).to.equal('1');
  });
