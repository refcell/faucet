...
abstract contract TVL is TrancheSystem, ERC1155PausableUpgradeable {
  ...
  function mint_item(
    ...
    if (_prevId > 0) {
      require(tokenIdExists[_prevId] == true, "Previous token id must exist.");
    }
    ...
    returns (bool)
  {
    ...
    for (uint256 i = 0; i < user_ids.length; i++) {
      ...
      for (uint256 x = 0; x < _ids.length; x++) {
        if (user_id == _ids[x]) {
          ...
          batch_amounts[counter] = tranche_id_amounts[user_level][user_id];
          counter++;
        }
      }
    }
    ...
    // * Batch transfer
    safeBatchTransferFrom(owner(), msg.sender, batch_ids, batch_amounts, _data);
